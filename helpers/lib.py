...
def get_metadata_from_infohash(info_hash_hex):
    # DHT routers now configured via settings pack (replaces add_dht_router)
    dht_routers = [
    ...
    settings = {
        ...
        'dht_bootstrap_nodes': ','.join(dht_routers),  # Modern way to add DHT routers
    }

    ses = lt.session(settings)
    ses.start_dht()

    # Create add_torrent_params using magnet URI
    ...
    while not metadata_received:
        # Use alerts instead of has_metadata() (modern approach)
        ...
        print(
            ...
            f"DHT nodes: {ses.status().dht_nodes} | "
